...
import { 
  ...
  Sparkles, Loader2, FileOutput, FolderOpen, Command, ChevronLeft, Grid, Undo2, Redo2, Plus, MonitorOff
} from 'lucide-react';
...
export const DocumentToolbar: React.FC<DocumentToolbarProps> = () => {
    ...
    const [activeMenu, setActiveMenu] = useState<'file' | 'edit' | 'view' | 'sort' | null>(null);
    ...
    const toggleMenu = (menu: 'file' | 'edit' | 'view' | 'sort') => {
        setActiveMenu(activeMenu === menu ? null : menu);
    };

    const handleMenuHover = (menu: 'file' | 'edit' | 'view' | 'sort') => {
        if (activeMenu) {
            setActiveMenu(menu);
        }
    };
    ...
    return (
        ...
        >
                ...
                >
                 ...
                 <div className="flex items-center mr-3 bg-black/5 dark:bg-white/5 rounded-md p-0.5">
                     ...
                     >
                        ...
                        {documents.length > 1 && <span className="opacity-50 font-normal ml-1">{activeDocIndex + 1}/{documents.length}</span>}
                     </button>
                    ...
                    {activeMenu === 'view' && (
                        <MenuDropdown onClose={closeMenu}>
                            <MenuItem 
                                ...
                                shortcut="Ctrl+K"
                             />
                            <MenuItem icon={MonitorOff} label="Toggle Zen Mode" onClick={() => { toggleZenMode(); closeMenu(); }} isActive={isZenMode}/>
                            <MenuDivider />
                             <MenuHeader label="Display Mode" />
                            <MenuItem icon={Grid} label="Pages View" onClick={() => { setViewMode('pages'); closeMenu(); }} isActive={viewMode === 'pages'}/>
            ...
            {/* Right Side: Theme & Template Name */}

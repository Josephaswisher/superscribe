...
export const DocumentView: React.FC<DocumentViewProps> = () => {
  ...
  const [slashContext, setSlashContext] = useState<{ header: string; lines: string[] } | null>(
    null
  );
  ...
  const handleSlashInput = (
    ...
    context?: { header: string; lines: string[] }
  ) => {
    ...
    if (!showSlashMenu) {
      ...
      if (value[cursorPos - 1] === '/') {
        ...
        // Approximate position (improved later)
        const rect = target.getBoundingClientRect();
        // This is very rough, assumes somewhat standard line height
        const lineHeight = 24;
        const lines = value.substr(0, cursorPos).split('\n');
        const currentLineIndex = lines.length - 1;

        setSlashMenuPosition({
          top: rect.top + currentLineIndex * lineHeight + 40, // Offset
          left: rect.left + 50, // Fixed left for now
        });
      }
    } else {
      // Update query
      const query = value.substring(slashTriggerIndex + 1, cursorPos);
      setSlashMenuQuery(query);

      // Close if space typed (optional, maybe we accept multi-word commands?)
      // or check if cursor moved before trigger
      if (cursorPos < slashTriggerIndex) {
  ...
  const handleSlashKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'Enter') {
      e.preventDefault(); // Prevent default navigation/newline if menu is open
      // Logic handled by SlashCommandMenu via global/shared state or specific event?
      // Wait, SlashCommandMenu is separate. We need to pass these key events TO the menu or handle them here?
      // The menu is rendered in DocumentView.
      // We can use a ref or just rely on the fact that if we preventDefault here, we need to manually trigger nav in menu.
      // Actually, SlashCommandMenu usually captures window events or we pass control.
      // My previous design had SlashCommandMenu listening? No.
      // Let's just prevent default here and assume SlashCommandMenu doesn't auto-listen.
      // We need a way to tell SlashCommandMenu to move selection.
      // For MVP: Let's just ensure Enter selects.
      // Arrow keys might be tricky without a ref to the menu.
      // Let's trigger a custom event or use a shared state for 'selectedIndex' if possible?
      // Or just let basic menu handle its own focus?
      // Actually, if textarea is focused, menu won't receive keys unless we trap them here.

      // FIXME: We need to change selected index in menu.
      // For now, let's just allow typing to filter.
      // Enter will pick the first item if we implement that logic.
      // I'll leave this simple for now: if Enter, check if we have a match?
      // For this step, just basic visibility is key.
  ...
  const executeSlashCommand = (cmd: SlashCommand) => {
    const target = activeTextarea || globalTextareaRef.current;
    if (!target) return;
    ...
    switch (cmd.id) {
      case 'vitals':
        insertion = '{{vitals}}'; // Will be processed
        break;
      case 'plan':
        insertion =
    ...
    // Apply Smart Template Processing
    if (slashContext && (cmd.id === 'vitals' || cmd.id === 'plan')) {
      insertion = processTemplate(insertion, slashContext.lines, slashContext.header);
    }

    // Replace the slash command query: from slashTriggerIndex to current cursor/selection
    const before = currentText.substring(0, slashTriggerIndex);
    const after = currentText.substring(cursorPos); // Assumption: cursor is at end of query

    const updated = before + insertion + after;

    // Update Textarea directly and dispatch event to bubble up to React
    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(
      window.HTMLTextAreaElement.prototype,
      'value'
    ).set;
    nativeInputValueSetter.call(target, updated);

    target.dispatchEvent(new Event('input', { bubbles: true }));

    // If global (fallback)
    if (target === globalTextareaRef.current) {

...
export const getDynamicPreambleHeader = (lines: string[], index?: number): string => {
    ...
    for (const line of lines.slice(0, 15)) {
        ...
        if (lower.match(/^(handoff|census|summary|assessment|plan|history|exam|vitals|labs|imaging|meds|disposition|###)/)) continue;

        // Pattern 1: **Name:** John Doe
        const nameLabelMatch = line.match(/(?:Name|Pt|Patient|PT NAME|Patient Name):\s*\*?\*?\s*([^*#\n-]+)/i);
        ...
        if (ageMatch && !age) {
            ...
            if (soloGenderMatch?.[1] && !gender) gender = (soloGenderMatch[1] as string).substring(0, 1).toUpperCase();
        }
...
export const parseRawContent = (content: string) => {
    ...
    return sections.map((section, index) => {
        ...
        const header = isInitialPreamble
            ? getDynamicPreambleHeader(lines)
            : `### ${lines[0]}`;
